<div class="flex h-16 shrink-0 items-center gap-2.5 px-5">
    <a href="{{ route('sites.index') }}" class="flex items-center gap-2.5 text-zinc-900">
        <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-teal-600 text-white">
            <x-application-logo class="h-4 w-4 fill-current" />
        </span>
        <span class="text-[15px] font-semibold tracking-tight">Analytics</span>
    </a>
</div>

<nav class="flex min-h-0 flex-1 flex-col px-3 pb-4">
    <div class="px-2 pb-1.5 pt-1 text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
        Workspace
    </div>
    <x-sidebar-link
        :href="route('sites.index')"
        icon="bx-grid-alt"
        :active="request()->routeIs('sites.index', 'dashboard')"
    >
        Sites
    </x-sidebar-link>
    <x-sidebar-link
        :href="route('sites.create')"
        icon="bx-plus-circle"
        :active="request()->routeIs('sites.create')"
    >
        Add site
    </x-sidebar-link>

    @if ($navSites->isNotEmpty())
        <div class="mt-5 px-2 pb-1.5 text-[11px] font-semibold uppercase tracking-wider text-zinc-400">
            Sites
        </div>
        <div class="min-h-0 flex-1 space-y-0.5 overflow-y-auto">
            @foreach ($navSites as $navSite)
                <x-sidebar-link
                    :href="route('sites.show', $navSite)"
                    icon="bx-globe"
                    :active="$currentSite && $currentSite->is($navSite)"
                    title="{{ $navSite->domain }}"
                >
                    {{ $navSite->name }}
                </x-sidebar-link>
            @endforeach
        </div>
    @endif
</nav>
